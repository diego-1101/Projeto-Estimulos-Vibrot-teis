...
def patch_app():
    ...
    if 'from performance_engine import' not in content:
        content = content.replace("from topoplot_engine import", "from performance_engine import run_normality_test, run_anova_typ3, plot_interactive_dot_sig, plot_interactive_interaction\nfrom topoplot_engine import")
    ...
    # We'll put it after get_topoplot_layout
    if 'def get_performance_layout():' not in content:
        # First, find get_topoplot_layout and its end
        # It usually ends before # --- Layout ---
        layout_func = """
...
"""
    ...
    # 5. Callbacks
    # We'll append them before the main block
...
def run_performance_analysis(n_clicks, prot, norm_chk, norm_var, norm_group,
    ...
    try:
        ...
        if not os.path.exists(csv_path):
            return dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, f"Arquivo não encontrado: {csv_path}"
        ...
        if 'yes' in (norm_chk or []):
            ...
            if norm_err: norm_out = html.Div(norm_err, className="alert alert-danger")
                ...
                def make_badge(is_norm): return html.Span("✅ Normal", className="badge bg-success ms-2") if is_norm else html.Span("❌ Não Normal", className="badge bg-danger ms-2")
                norm_out = html.Div([
                    html.H5(f"Testando: {norm_var} | Grupo: {norm_group if prot in ['A','B'] else 'N/A'}", className="mb-3"),
                    html.Div([html.Strong("Shapiro-Wilk:"), html.Span(f" W = {shapiro_res['stat']:.4f}, p = {shapiro_res['p_value']:.4e}", className="ms-2"), make_badge(shapiro_res['is_normal'])], className="mb-2"),
                    html.Div([html.Strong("Kolmogorov-Smirnov:"), html.Span(f" D = {ks_res['stat']:.4f}, p = {ks_res['p_value']:.4e}", className="ms-2"), make_badge(ks_res['is_normal'])], className="mb-2")
                ])
...
def update_posthoc_plot(table_data, active_cell, prot, target_var, alpha, fase_val):
    ...
    if active_cell: selected_row_idx = active_cell['row']
    else:
        for i, row in enumerate(table_data):
            if row.get('Termo') not in ['Intercept', 'Residual']:
                try:
        ...
        if selected_row_idx is None:
            for i, row in enumerate(table_data):
                if row.get('Termo') not in ['Intercept', 'Residual']:
                    selected_row_idx = i
                    break
    if selected_row_idx is None: return go.Figure(), "Nenhum fator analisável encontrado."
    source_str = table_data[selected_row_idx]['Termo']
    import re
    ...
    if err: posthoc_out = html.Div(err, className="alert alert-warning")
    elif not sig_results: posthoc_out = html.Div("Nenhuma diferença significativa encontrada no post-hoc.", className="alert alert-info")
    else:
...
if __name__ == '__main__':
    patch_app()
    print("Re-applied performance tab to app.py")
